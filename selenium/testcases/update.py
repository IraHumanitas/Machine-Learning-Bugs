...
def run_update_test(driver, url, test_cases):
    driver.get(url)
    results = []
    ...
    for case in test_cases:
        ...
        for field, selector_type in case['selector_input'].items():
            if element_exists(driver, selector_type, field):
                ...
                if case['input_action'].get(field):
                    element.clear()
                    element.send_keys(case['input_action'][field])
                elif existing_value:
                    element.send_keys(existing_value)
                else:
                    continue
            else:
                has_bug = True
